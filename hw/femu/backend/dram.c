...
int backend_rw(SsdDramBackend *b, QEMUSGList *qsg, uint64_t *lbal, bool is_write, bool is_lower_layer, uint32_t container_id)
{
    int sg_cur_index = 0;
    ...
    if (is_write) {
        ...
        ssd_write(ssd, lpn, size, container_id, is_lower_layer);
    }
