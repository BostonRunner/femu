...
extern void replay_trace_file(struct ssd *ssd, const char *filename);
...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    // 仅当设置 trace-path 时，执行初始化和重放
    if (n->trace_path) {
        femu_debug("[FEMU] Initializing SSD for trace playback: %s\n", n->trace_path);

        // 初始化 SSD 核心结构（内部会设置 n->ssd）
        bb_init(n, errp);
        if (!n->ssd) {
            fprintf(stderr, "[FEMU] SSD initialization failed!\n");
            return;
        }
...
static void femu_exit(PCIDevice *pci_dev)
{
    ...
    print_detailed_statistics(n->ssd, "stats.log");
    femu_debug("SSD statistics written to stats.log\n");
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_STRING("trace-path", FemuCtrl, trace_path),
